...
def floorCoords(args, n):
    """Flatten args, floor the first n values (the x/y/z coordinates) and pass
    everything after them (the block id and optional data) through unchanged.

    This is what lets a modern block name (a string such as "copper_block")
    survive the trip to the server: the classic code floored *every* argument,
    which crashed on string ids. Coordinates are still floored exactly as
    before, so numeric-id lessons behave identically."""
    flat = list(flatten(args))
    coords = [int(math.floor(float(x))) for x in flat[:n]]
    return coords + flat[n:]
...
class Minecraft:
    ...
    def getBlockName(self, *args):
        """Get the modern namespaced block name (x,y,z) => str

        Returns e.g. "minecraft:copper_block". Use this to read back modern
        blocks that have no classic numeric id (getBlock would report 0/air
        for those)."""
        return self.conn.sendReceive(b"world.getBlockName", intFloor(args))
    ...
    def setBlock(self, *args):
        ...
        id may be a classic numeric id (int) or a modern block name (str),
        e.g. mc.setBlock(x, y, z, block.COPPER_BLOCK)."""
        self.conn.send(b"world.setBlock", floorCoords(args, 3))

    def setBlocks(self, *args):
        """Set a cuboid of blocks (x0,y0,z0,x1,y1,z1,id,[data])

        id may be a classic numeric id (int) or a modern block name (str)."""
        self.conn.send(b"world.setBlocks", floorCoords(args, 6))
